    ...
    getters: {
      filteredItems(state) {
        return state.listingItems.filter(item => {
          const matchesSearchTerm = item.city.toLowerCase().includes(state.filter.searchTerm.toLowerCase());
          const matchesPriceRange = item.price >= state.filter.priceRange[0] && item.price <= state.filter.priceRange[1];
          const matchesTypes = state.filter.selectedTypes.length === 0 || state.filter.selectedTypes.includes(item.type);
